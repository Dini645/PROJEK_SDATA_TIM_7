...
bool isValidCustomerCode(const char* code) {
    // Kode pelanggan harus 3-6 karakter (huruf/angka)
...
TreeNode::TreeNode(const char* n, const char* code, int a) : amount(a), height(1), freq(0), ch('\0'), left(NULL), right(NULL) {
    copyString(name, n, 50);
    copyString(this->customerCode, code, 10);
}
...
Transaction::Transaction(int num, const char* name, const char* code, int total, int food, 
    ...
    copyString(customerCode, code, 10);
...
Customer::Customer(const char* n, const char* code, int t) : totalSpent(t), visitCount(1), next(NULL) {
    copyString(name, n, 50);
    copyString(this->customerCode, code, 10);
}
...
void ShoppingCart::display() {
    ...
    if (stringLength(customerCode) > 0) {
        cout << "Kode Pelanggan : " << customerCode << "\n";
    }
...
void ShoppingCart::setCustomerCode(const char* code) {
    copyString(customerCode, code, 10);
    cout << ">> Kode pelanggan diset menjadi: " << code << "\n";
}

void ShoppingCart::getCustomerCode(char* code) {
    copyString(code, customerCode, 10);
}
...
void TransactionQueue::displayQueue() {
    ...
    cout << "\nNo  No.Pes  Nama Pelanggan        Total       Kode\n";
    cout << "--------------------------------------------------\n";
    ...
    while (temp != NULL) {
        ...
        if (stringLength(temp->transaction->customerCode) > 0) {
            cout << temp->transaction->customerCode;
        } else {
            cout << "Tanpa Kode";
        }
        cout << "\n";
...
void CustomerBinaryTree::insertRecursive(TreeNode* &node, const char* name, const char* code, int amount) {
    if (node == NULL) {
        node = new TreeNode(name, code, amount);
        return;
    }
    
    // Bandingkan berdasarkan kode pelanggan
    int i = 0;
    while (code[i] != '\0' && node->customerCode[i] != '\0') {
        if (code[i] < node->customerCode[i]) {
            insertRecursive(node->left, name, code, amount);
            return;
        } else if (code[i] > node->customerCode[i]) {
            insertRecursive(node->right, name, code, amount);
            return;
        }
        i++;
    }
    ...
    // Jika kode sama
    if (code[i] == '\0' && node->customerCode[i] == '\0') {
...
void CustomerBinaryTree::insert(const char* name, const char* code, int amount) {
    insertRecursive(root, name, code, amount);
}
...
void CustomerBinaryTree::displayInOrder(TreeNode* node) {
    ...
    cout << "Nama: " << node->name << ", Kode: " << node->customerCode 
...
TreeNode* CustomerBinaryTree::searchRecursive(TreeNode* node, const char* code) {
    if (node == NULL) return NULL;
    
    if (stringsEqual(node->customerCode, code)) return node;
    
    TreeNode* leftResult = searchRecursive(node->left, code);
    if (leftResult != NULL) return leftResult;
    
    return searchRecursive(node->right, code);
}

TreeNode* CustomerBinaryTree::search(const char* code) {
...
TreeNode* CustomerAVLTree::insertRecursive(TreeNode* node, const char* name, const char* code, int amount) {
    if (node == NULL) return new TreeNode(name, code, amount);
    ...
    if (compare < 0)
        node->left = insertRecursive(node->left, name, code, amount);
    else
        node->right = insertRecursive(node->right, name, code, amount);
...
void CustomerAVLTree::insert(const char* name, const char* code, int amount) {
    root = insertRecursive(root, name, code, amount);
}
...
TreeNode* CustomerAVLTree::searchRecursive(TreeNode* node, const char* code) {
    if (node == NULL) return NULL;
...
TreeNode* CustomerAVLTree::search(const char* code) {
    return searchRecursive(root, code);
}
...
void TransactionHistory::addTransaction(int orderNum, const char* customerName, const char* code, int total, 
                                       int food, int drink, const char* date) {
    Transaction* newTrans = new Transaction(orderNum, customerName, code, total, food, drink, date);
    newTrans->next = head;
    head = newTrans;
    ...
    if (stringLength(customerName) > 0 && stringLength(code) > 0) {
        addToCustomerTree(customerName, code, total);
        addToCustomerAVL(customerName, code, total);
    }
}
...
void TransactionHistory::displayAllTransactions() {
    ...
    cout << "\n--------------------------------------------------------------------------------\n";
    cout << "No  No.Pes  Nama Pelanggan        Kode       Total    Makan Minum Tanggal\n";
    cout << "--------------------------------------------------------------------------------\n";
    ...
    while (temp != NULL) {
        ...
        if (stringLength(temp->customerCode) > 0) {
            cout << temp->customerCode;
            int codeLen = stringLength(temp->customerCode);
            for(int i = codeLen; i < 12; i++) cout << " ";
        } else {
            cout << "Tanpa Kode   ";
        }
...
void TransactionHistory::saveTransactionsToFile() {
    ...
    if (file.is_open()) {
        ...
        while (temp != NULL) {
            ...
            file << "Kode         : " << (stringLength(temp->customerCode) > 0 ? temp->customerCode : "Tanpa Kode") << "\n";
...
Transaction* TransactionHistory::searchTransactionByCode(const char* code) {
    Transaction* temp = head;
    while (temp != NULL) {
        if (stringsEqual(temp->customerCode, code)) {
            return temp;
        }
        temp = temp->next;
    }
...
void TransactionHistory::searchTransactionByDate(const char* date) {
    ...
    cout << "\nNo.Pes  Nama Pelanggan        Kode       Total    Makan Minum\n";
    cout << "--------------------------------------------------------------------\n";
...
void TransactionHistory::searchTransactionByAmountRange(int minAmount, int maxAmount) {
    ...
    cout << "\nNo.Pes  Nama Pelanggan        Kode       Total    Makan Minum Tanggal\n";
    cout << "------------------------------------------------------------------------------\n";
...
void TransactionHistory::displayTransactionDetails(Transaction* trans) {
    ...
    cout << "Kode Pelanggan : " << (stringLength(trans->customerCode) > 0 ? trans->customerCode : "Tanpa Kode") << "\n";
...
void TransactionHistory::searchTransactionMenu() {
    ...
    do {
        ...
        cout << "3. Cari Berdasarkan Kode Pelanggan\n";
        ...
        switch (searchChoice) {
            ...
            case 3: {
                ...
                if (!isValidCustomerCode(code)) {
                    cout << "\n>> Format kode tidak valid! Kode harus 3-6 karakter (huruf/angka).\n";
                    break;
                }
                
                Transaction* found = searchTransactionByCode(code);
                if (found != NULL) {
                    cout << "\n>> Transaksi ditemukan untuk kode " << code << ":\n";
                    displayTransactionDetails(found);
                    
                    cout << "\n>> Semua transaksi dari kode " << code << ":\n";
                    cout << "--------------------------------------------------\n";
                    Transaction* temp = head;
                    int transCount = 0;
                    int totalSpent = 0;
                    ...
                    cout << "\n>> Data pelanggan dengan kode " << code << ":\n";
                    cout << "Nama: " << found->customerName << "\n";
                    cout << "Total pengeluaran: Rp " << totalSpent << "\n";
                    cout << "Frekuensi pesanan: " << transCount << "\n";
                } else {
                    cout << "\n>> Tidak ditemukan transaksi untuk kode " << code << "\n";
                }
                break;
            }
...
void TransactionHistory::processPendingQueue() {
    ...
    while (!pendingQueue.isEmpty()) {
        ...
        cout << ">> Memproses transaksi #" << trans->orderNumber 
             << " - " << trans->customerName << " (" << trans->customerCode 
...
void TransactionHistory::addToCustomerTree(const char* name, const char* code, int amount) {
    customerTree.insert(name, code, amount);
}

void TransactionHistory::addToCustomerAVL(const char* name, const char* code, int amount) {
    customerAVL.insert(name, code, amount);
}
...
void CustomerManager::addCustomer(const char* name, const char* code, int total) {
    Customer* existing = findCustomerByCode(code);
    if (existing != NULL) {
        ...
        if (!stringsEqual(existing->name, name)) {
            cout << ">> Info: Nama pelanggan diupdate dari '" << existing->name 
                 << "' ke '" << name << "' untuk kode " << code << "\n";
            copyString(existing->name, name, 50);
        }
    } else {
        Customer* newCustomer = new Customer(name, code, total);
...
void CustomerManager::displayAllCustomers() {
    ...
    cout << "\n--------------------------------------------------------------------------------\n";
    cout << "No  Nama Pelanggan           Kode     Total Belanja Kunjungan\n";
    cout << "--------------------------------------------------------------------------------\n";
    ...
    if (topSpender) {
        cout << "Top Spender        : " << topSpender->name 
             << " (" << topSpender->customerCode << ") - Rp " << topSpender->totalSpent << "\n";
    }
...
Customer* CustomerManager::findCustomerByCode(const char* code) {
    Customer* temp = head;
    while (temp != NULL) {
        if (stringsEqual(temp->customerCode, code)) {
            return temp;
...
void CustomerManager::searchCustomerByName(const char* name) {
    ...
    if (found != NULL) {
        ...
        cout << "Kode Pelanggan : " << found->customerCode << "\n";
...
void CustomerManager::searchCustomerByCode(const char* code) {
    Customer* found = findCustomerByCode(code);
    if (found != NULL) {
        cout << "\n==================================================\n";
        cout << "           DATA PELANGGAN DITEMUKAN (BERDASARKAN KODE)\n";
        cout << "==================================================\n";
        cout << "Kode Pelanggan : " << found->customerCode << "\n";
        cout << "Nama Pelanggan : " << found->name << "\n";
        cout << "Total Belanja  : Rp " << found->totalSpent << "\n";
        ...
        cout << "\n>> Pelanggan dengan kode '" << code << "' tidak ditemukan.\n";
    }
}
...
void DriveThruSystem::displayCustomerMenu() {
    ...
    cout << "\nOPSI PELANGGAN:\n";
    cout << "1. Lihat Menu\n";
    cout << "2. Masukkan Data Diri (Nama & Kode Pelanggan)\n";
...
void DriveThruSystem::printReceipt(int orderNum, const char* customerName, const char* code, int total, 
                                 int food, int drink) {
    cout << "\n==================================================\n";
            ...
            case 5:
                clearScreen();
                {
                    ...
                    if (searchChoice == 1) {
                        ...
                        char searchCode[10];
                        cout << "\nMasukkan kode pelanggan yang dicari: ";
                        cin.ignore();
                        cin.getline(searchCode, 10);
                        customers.searchCustomerByCode(searchCode);
                    }
                }
                break;
    ...
    do {
        ...
        switch (choice) {
            ...
            case 2:
                ...
                cout << "Kode pelanggan (3-6 karakter, huruf/angka): ";
                cin.getline(code, 10);
                
                if (isValidCustomerCode(code)) {
                    cart.setCustomerCode(code);
                    cout << ">> Kode pelanggan valid dan berhasil disimpan.\n";
                    cout << ">> Contoh kode yang baik: ABC123, USER01, CUSTA\n";
                } else {
                    cout << ">> Format kode tidak valid! Harus 3-6 karakter (huruf/angka).\n";
                    cout << ">> Contoh kode yang valid: ABC, 123, USER1, CUSTA\n";
            ...
            case 7:
                ...
                if (cart.isEmpty()) {
                    ...
                    if (confirm == 'y' || confirm == 'Y') {
                        ...
                        char customerName[50];
                        char customerCode[10];
                        cart.getCustomerName(customerName);
                        cart.getCustomerCode(customerCode);
                        
                        char date[20];
                        copyString(date, "Sistem Drive-Thru", 20);
                        
                        transactions.addTransaction(orderCounter, customerName, customerCode, total, food, drink, date);
                        
                        if (stringLength(customerName) > 0 && stringLength(customerCode) > 0) {
                            customers.addCustomer(customerName, customerCode, total);
                        } else {
                            cout << "\n>> Catatan: Data pelanggan tidak lengkap, hanya transaksi yang disimpan.\n";
                        }
                        
                        printReceipt(orderCounter, customerName, customerCode, total, food, drink);
...
int main() {
    ...
    cout << "==================================================\n";
    cout << "       SISTEM DRIVE-THRU FAST FOOD\n";
    cout << "       DENGAN FITUR PENCARIAN PESANAN & KODE PELANGGAN\n";
    cout << "==================================================\n";
    cout << "Fitur Utama:\n";
    cout << "1. Sistem Pemesanan Drive-Thru dengan Kode Pelanggan (3-6 karakter)\n";
    cout << "2. Pencarian Pesanan berdasarkan Nama/Kode Pelanggan\n";
